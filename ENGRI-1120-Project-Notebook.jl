...
# ╔═╡ 2f3d84b8-012e-45a6-bffb-4ad2677c73b4
html"""
<p style="font-size:20px;"> 
We, as a team, are endorsed by a client to produce isoprene with 95% purity from maltose at a rate of 1g/hr. Our goal is to produce a feasible solution to this production and determine whether this solution is economically viable (profitable).
</br>
</br>
Isoprene is mostly produced to prepare for production of polymers and copolymers. These polymers are synthesized to a material that resembles natural rubber(butyl rubber) in both chemical structure and properties. Isoprene is also used in the pharmaceutical industry as an intermediate in vitamins. It has also taken a part in perfumes, flavorings, and even epoxy hardeners.  
</br>
(source: https://pubchem.ncbi.nlm.nih.gov/compound/Isoprene#section=Use-and-Manufacturing) 
</br>
</br>
Our client supplies us with maltose, which is a disaccharide formed by two units of glucose joining each other via an alpha glycosidic linkage. Maltose is commonly found in germinating grains (seeds) and other parts of plants and in certain foods, like cereal and sweet potatoes. It is also produced as an intermediate product of the digestion process of glycogen and starch.
</br>
</br>
Assumptions made: water is free, all streams are ideal, at atmospheric pressure, isothermal processes, reacting at optimal temerature, pump has negligible power consumption and can be fed by infinite volumes of syringes
...
begin
	...
	compounds_that_we_need_to_supply_feed_2 = [
		"3-phospho-d-glyceroyl phosphate", "1-deoxy-d-xylulose 5-phosphate"
	]

	mol_flow_values_feed_2 = [
		2.47933  ; #3-phospho-d-glyceroyl phosphate mmol/hr
		2.4595   ;#1-deoxy-d-xylulose 5-phosphate mmol/hr
	]
...
# ╔═╡ 9d327971-7d9a-4a38-94e3-53c4154d22ba
...
# ╔═╡ 0f5fd001-ec2a-425a-87d7-5c5aaf960eb0
begin
	...
	plot(stages,species_mass_fraction_array_top[idx_target_compound,:], linetype=:steppre,lw=2,legend=:bottomright, 
		label="Mass fraction i = Isoprene Tops")
...
# ╔═╡ 325d458c-82a6-47e4-abd1-b9e2acc9fe9e
with_terminal() do
...
# ╔═╡ daa3eeb5-e155-4a5a-90b2-4dd259c8d09d
...
# expected payoff time: 3 future years (period of time to payback the investment)
T = 3

# ╔═╡ f47f09f3-210d-4e8e-8efb-6703596f5054
#assuming discount rate is 1% per year
discount_rate = 0.01

# ╔═╡ 40d063fd-0e94-47ca-b731-b31e96e0d0b7
# Setup the cash flow array -
# the upfront cost paid at the bigging of process: 3620$
# we save in year(s) 2,3 and 4: 90K (in,+ve)
CF_array = [-3.62, -14319865.3, -14319865.3, -14319865.3]

# ╔═╡ de5bebc4-7c81-4ed9-87df-4292974b1dd0
...
# ╔═╡ 80a31694-525f-438a-957c-b403099f62ab
# what is the PV?
PV = CF_array.*(1.0./discount_array)

# ╔═╡ 3fc2032e-7c1c-4b92-97ad-822df87b43c1
# what is the NPV?
NPV = sum(PV)

# ╔═╡ 36448b75-cfe7-4867-8be1-b3624c89e272


# ╔═╡ 503a8717-6be1-4a43-8252-b19bf30e2880


# ╔═╡ ecf643e4-42af-4679-9213-d271855061ec
#assuming discount rate is 10% per year
discount_rate_10 = 0.1

# ╔═╡ 6524492b-0af1-4ac2-a529-5ad50761706b
# Setup the cash flow array -
# we pay in year 1: $230K (out, -ve)
# we save in year(s) 2,3 and 4: 90K (in,+ve)
CF_array_10 = [-3.62, -14319865.3, -14319865.3, -14319865.3]

# ╔═╡ 75c31353-c076-437d-b3fd-02bdf97cf65f
# compute the discount terms -
begin
	
	# main -
	discount_array_10 = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_10)^(time_index-1)
    	discount_array_10[time_index] = tmp_term
	end
...
# ╔═╡ 47c5a5de-3648-4cde-95a3-c3f6badab004
# what is the PV?
PV10 = CF_array_10.*(1.0./discount_array_10)

# ╔═╡ 4cb651e7-7cc8-4767-ade0-67ff38869f90
# what is the NPV?
NPV10 = sum(PV10)

# ╔═╡ 63840773-e3f9-4179-a68f-29ea352f0276


# ╔═╡ 3ec6c246-4901-4f0f-8e23-a17d0c0d162d


# ╔═╡ 62049adc-6616-406b-87ae-85a61277abf4
md"""
### Alternative Solution:
"""

# ╔═╡ 61a85ef9-a795-466a-a257-157fdc9c8ac1
md"""
Using maltose to make isoprene is not an economically feasible option as the investments significantly exceed the returns of the process. 

For an alternative approach, we tested 6.165 mmol/hr of d-glyceraldehyde 3-phosphate in stream 1 and 2.466 mmol/hr of pyruvate in stream 2. Using 25 chips and 6 levels, 1.02364 mmol of isoprene was produced. The upfront cost of the splitter, chips, and separator units is 3,620 USD.
Every year there is a loss of 7,417,577,547.55 USD which is less than a loss of ~14billion USD when using maltose. However, neither approach is profitable.
"""

# ╔═╡ fa3f95bb-88e9-44f4-8bfe-559b429090df
...
#assuming discount rate is 1% per year
discount_rate_1_alt = 0.01

# ╔═╡ 6ad17a92-4e39-4d18-8ddb-1a97411b6717
# Setup the cash flow array -
# the upfront cost paid at the bigging of process: $3.62K (out, -)
# spendings in year 1, 2, 3: $7417577.5K (out, -)
CF_array_1_alt = [-3.62, -7417577.5, -7417577.5, -7417577.5]

# ╔═╡ 4008ace9-6529-4fa7-938f-1b26dbfcbb1d
# compute the discount terms -
begin
	
	# main -
	discount_array_1_alt = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_1_alt)^(time_index-1)
    	discount_array_1_alt[time_index] = tmp_term
	end
...
# ╔═╡ df09af13-ba07-4e8b-b9f1-135a3bae8a85
# what is the PV?
PV1_alt = CF_array_1_alt.*(1.0./discount_array_1_alt)

# ╔═╡ 5e66ed9c-78e2-4786-a5d9-ab9ac5f49b8b
# what is the NPV?
NPV1_alt = sum(PV1_alt)

# ╔═╡ 5b5f6d79-fffa-4d0c-8706-e3f90f0ba52d


# ╔═╡ a9ddb0d0-ebeb-4e46-828c-13c09d0416be


# ╔═╡ 3758af2e-3056-486f-ad47-3fdd1d3b5b5a
#assuming discount rate is 10% per year
discount_rate_10_alt = 0.1

# ╔═╡ feb2a734-d694-4fbf-a85f-2c6831fe1c07
# Setup the cash flow array -
# the upfront cost paid at the bigging of process: $3.62K (out, -)
# spendings in year 1, 2, 3: $7417577.5K (out, -)
CF_array_10_alt = [-3.62, -7417577.5, -7417577.5, -7417577.5]

# ╔═╡ a854ba0e-2b75-4948-9bea-ce56eddae239
# compute the discount terms -
begin
	
	# main -
	discount_array_10_alt = ones(T+1)
	for time_index = 2:(T+1)
    	tmp_term = (1+discount_rate_10_alt)^(time_index-1)
    	discount_array_10_alt[time_index] = tmp_term
	end
...
# ╔═╡ ae0acee8-28a3-4d11-8f85-43242e474f73
# what is the PV?
PV10_alt = CF_array_10_alt.*(1.0./discount_array_10_alt)

# ╔═╡ 1bad844c-ed6f-45ad-a369-da9286d12e55
# what is the NPV?
NPV10_alt = sum(PV10_alt)

# ╔═╡ 2fcec245-4ecc-4320-8498-a80189752178
...
For our original data, consisting of maltose, 1-deoxy-d-xylulose 5-phosphate, and 3-phospho-d-glyceroyl phosphate results in a negative NPV value. Using these inputs, the project of creating 1g/hr of >=95% isoprene is not economically feasible(NPV<0). We also have multiple alternative proposals, but the best one(most “profitable”) is by using d-glyceraldehyde 3-phosphate and pyruvate as our only inputs. Even though maltose is removed from the process, glycolysis still exists! This helps limit the amount of materials needed to create isoprene, which further limits the cost and spending of isoprene production. Overall, these results in a less (still)negative value of NPV than before, which leads us closer to becoming financially feasible. However, realistically, this project is not feasible economically and we don’t recommend attempting to use Maltose/Products of Maltose to create Isoprene. 
"""
...
# ╔═╡ 876eb74d-0ad1-44fb-b069-97325adb98ba
html"""
<a href="url">https://www.sciencedirect.com/topics/medicine-and-dentistry/maltose</a>
"""

# ╔═╡ cc6502a2-cd0a-4e7d-905f-b626883bcbf2

html"""
<a href="url">https://pubchem.ncbi.nlm.nih.gov/compound/Isoprene#section=Use-and-Manufacturing</a>
"""

# ╔═╡ 81824d2a-820b-4ddc-a7ad-255184787f1a

html"""
<a href="url">https://www.sigmaaldrich.com/US/en</a>
"""
